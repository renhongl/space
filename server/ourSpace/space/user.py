...
def api(request):
    if request.method == 'GET':
        ...
        if UserModel.objects.filter(userName=request.GET['userName']).filter(userPassword=request.GET['password']).exists():
            query_user = UserModel.objects.filter(
                userName=request.GET['userName']).first()
        ...
        if UserModel.objects.filter(userName=postData['userName']).exists():
            ...
            create_user.save()
            return HttpResponse(json.dumps({'status': '00000000', 'message': '注册成功。'}), content_type='application/json')
